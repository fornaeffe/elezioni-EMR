...
partendo dai risultati delle ultime elezioni politiche e del sondaggio BiDiMedia 15 novembre 2024.
Vengono eseguite 10000 simulazioni, in ciascuna di esse le percentuali stimate
dal sondaggio BiDiMedia vengono distribuite tra le province secondo la distribuzione
verificata alle ultime elezioni politiche, e poi queste percentuali
...
```{r sondaggi}
sondaggi <- read_xlsx("dati/liste.xlsx", "sondaggi")
sondaggi$DATA <- as.POSIXct(sondaggi$DATA)

astensione_base <- lista_elezione$PERCENTUALE[
  lista_elezione$LISTA == "astensione" &
    lista_elezione$DATA == max(lista_elezione$DATA)
]

sondaggi <- rbindlist(tapply(
  sondaggi,
  interaction(sondaggi$SONDAGGIO, sondaggi$DATA),
  function(df) {
    if ("astensione" %in% df$LISTA) {
      astensione <- df$PERCENTUALE[df$LISTA == "astensione"]
    } else {
      astensione <- astensione_base
    }

    df$PERCENTUALE[df$LISTA != "astensione"] <-
      df$PERCENTUALE[df$LISTA != "astensione"] * (1 - astensione)
...
sondaggi$LOG_P <- qlogis(sondaggi$PERCENTUALE)

lista_elezione <- rbind(
  lista_elezione,
  data.frame(
    ELEZIONE = sondaggi$SONDAGGIO,
    LISTA = sondaggi$LISTA,
    VOTI_LISTA = NA,
    VOTI_LISTA_REG = NA,
    PERCENTUALE = sondaggi$PERCENTUALE,
    LOG_P = sondaggi$LOG_P,
    DATA = sondaggi$DATA
  )
)
...
SIGMA_P <- tapply(prov_lista_elezione, prov_lista_elezione$LISTA, function(df) {
  sd(unlist(tapply(df, df$PROVINCIA, function(df) {
    df <- df[order(df$DATA),]
    return(diff(df$DELTA) / unclass(diff(df$DATA)))
  })))
})
...
prov_lista <- prov_lista_elezione[,
  c("ELEZIONE", "LISTA", "PROVINCIA", "DELTA", "VOTI_LISTA_PROV", "DATA")
]
prov_lista <- prov_lista[order(prov_lista$ELEZIONE == "camera_2022", prov_lista$DATA, decreasing = TRUE),]
prov_lista <- prov_lista[!duplicated(interaction(prov_lista$LISTA, prov_lista$PROVINCIA)),]
prov_lista$ELEZIONE <- NULL
# prov_lista <- rbind(
#   prov_lista,
#   prov_lista_elezione[
...
elettori <- prov_lista[prov_lista$LISTA == "PARTITO DEMOCRATICO", c("PROVINCIA", "VOTI_LISTA_PROV")]

prov_lista$VOTI_LISTA_PROV <- NULL

prov_lista <- merge(prov_lista, elettori)

prov_lista$DATA <- max(prov_lista$DATA)
