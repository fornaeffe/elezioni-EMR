...
prepara_voti <- function() {
  a <- aggregate(
    VOTI_LISTA ~ DATA + PROVINCIA + LISTA,
    prov_lista_elezione,
    sum,
    drop = FALSE
  )
  ...
  c <- round(abind(
    lapply(b, function(l) abind(split(l$VOTI_LISTA, l$PROVINCIA), along = 2)),
    along = 3
  ))
...
frazione_base_init <- function(voti) {
  percentuali <- apply(voti, c(1, 2), function(riga) riga / sum(riga))
  percentuali_medie <- apply(percentuali, c(1, 3), mean)
  return(t(percentuali_medie))
}
...
passaggi_init <- function(E, P, L) {
  diagonale <- diag(L)
  alpha <- ifelse(diagonale == 1, 100, 1)
  beta <- ifelse(diagonale == 1, 1, 100)
  
  genera_matrice <- function() {
    matrice <- matrix(rbeta(L*L, alpha, beta), L, L)
    matrice <- apply(matrice, 1, function(riga) riga / sum(riga))
  }
  
  risultato <- aperm(replicate(E-1, replicate(P, genera_matrice())), c(4, 3, 2, 1))
  
  risultato
}

matrice_init <- function(L, mu_stesso, mu_diverso) {
  matrice <- diag(L) * mu_stesso
  matrice[matrice == 0] <- mu_diverso
  matrice
}

matrice_passaggi_init <- function(E, L, mu_stesso, mu_diverso) {
  risultato <- aperm(replicate(E-1, matrice_init(L, mu_stesso, mu_diverso)), c(3, 1, 2))
  risultato
}
...
fit <- stan(
  ...
  init = function() {
    list(
      frazione_base = frazione_base_init(dati$voti),
      passaggi = passaggi_init(dati$E, dati$P, dati$L),
      matrice_passaggi = matrice_passaggi_init(dati$E, dati$L, dati$mu_stesso, dati$mu_diverso),
      matrice_base = matrice_init(dati$L, dati$mu_stesso, dati$mu_diverso)
    )
  }
)
...
save(fit, file="stanfit-23062024.RData")
