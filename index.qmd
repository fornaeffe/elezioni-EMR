...
prov_lista <- aggregate(
  LOG_P ~ PROVINCIA + LISTA,
  prov_lista_elezione,
  sd
)

prov_lista$LOG_P[is.na(prov_lista$LOG_P)] <- mean(prov_lista$LOG_P, na.rm = TRUE)
...
names(liste)[names(liste) == "LOG_P"] <- "variab"
...
prov_lista <- read_xlsx("dati/liste.xlsx", "europee")

stopifnot(setequal(liste$LISTA, c(unique(prov_lista$LISTA), "astensione")))
...
prov_lista$LOG_P_BASE <- log(prov_lista$VOTI_LISTA / prov_lista$ELETTORI)

prov_lista <- merge(
  prov_lista,
  liste[, c("LISTA", "variab")]
)
...
simula <- function(
  ...
  iterazione <- function(
    ...
    prov_lista$VOTI_LISTA_ITER <- prov_lista$ELETTORI * prov_lista$PERCENTUALE_ITER
...
boxplot(
  ...
  xlab = "PERCENTUALE",
  ylab = NA,
  col = liste$COLORE[order(liste$LISTA)]
  )
