...
lista_elezione <- lista_elezione[order(lista_elezione$LISTA),]

dati <- list(
  L = nrow(liste),
  NR = nrow(differenze),
  diffR = differenze$DIFFERENZA,
  tempo = as.numeric(differenze$DELTA_T) / 365.25,
  lista = as.numeric(differenze$LISTA),
  lPR = lista_elezione$LOG_P[lista_elezione$ELEZIONE == "europee2024"],
  tempo_prox = as.numeric(delta_t) / 365.25
)
...
boxplot(plogis(fit_ext$lPR_new), horizontal = TRUE)
plot(density(plogis(fit_ext$lPR_new[,1])))

apply(plogis(fit_ext$lPR_new),2,mean)

percentuali <- apply(plogis(fit_ext$lPR_new), 1, function(riga) riga[1:12] / sum(riga[1:12]))
apply(percentuali,1,median)
```
...
md_L <- lm(abs(DIFFERENZA) ~ LISTA, data = differenze)
md_L_T <- lm(abs(DIFFERENZA) ~ LISTA + DELTA_T, data = differenze)
md_LT <- lm(abs(DIFFERENZA) ~ LISTA : DELTA_T, data = differenze)
md_L_LT <- lm(abs(DIFFERENZA) ~ LISTA + LISTA : DELTA_T, data = differenze)
...
md_1 <- lm(abs(DIFFERENZA) ~ 1, data = differenzeP)
md_1_T <- lm(abs(DIFFERENZA) ~ DELTA_T, data = differenzeP)
md_T <- lm(abs(DIFFERENZA) ~ DELTA_T - 1, data = differenzeP)
md_L <- lm(abs(DIFFERENZA) ~ LISTA, data = differenzeP)
md_L_T <- lm(abs(DIFFERENZA) ~ LISTA + DELTA_T, data = differenzeP)
md_LT <- lm(abs(DIFFERENZA) ~ LISTA : DELTA_T, data = differenzeP)
md_L_LT <- lm(abs(DIFFERENZA) ~ LISTA + LISTA : DELTA_T, data = differenzeP)
...
plot(abs(DIFFERENZA) ~ DELTA_T, data = differenzeP)
abline(md_1_T)
