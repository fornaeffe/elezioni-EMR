...
# Carico lo scenario
load(paste0("scenari/", scenario, ".RData"))

elezioni <- unique(dati[,.(DATA, ELEZIONE)])
...
# Appiattisco la matrice di corrispondenza tra liste precedenti e attuali
setnames(matrice_liste, "LISTA", "LISTA_ORIGINALE")
corrispondenza_liste <- melt(
  matrice_liste,
  c("DATA", "ELEZIONE", "LISTA_ORIGINALE"),
  variable.name = "LISTA",
  value.name = "FATTORE"
)

# Rimuovo le righe inutili
corrispondenza_liste <- corrispondenza_liste[FATTORE != 0]
...
calcola_sigma <- function(df) return(
  sd(diff(df$LOGIT_P) / unclass(diff(df$DATA)))
)

liste <- liste[
  lista_elezione[
    ,
    .(SIGMA_R = calcola_sigma(.SD)),
    by = .(LISTA)
  ],
  on = .(LISTA)
]
